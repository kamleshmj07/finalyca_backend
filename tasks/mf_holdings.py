...
import os
import pandas as pd
from bizlogic.importer_helper import lookup_sebi_sector_using_sebi_industry, save_issuer, get_rel_path
...
def import_mf_holding_file(input_file_path, output_file_path, user_id):
    ...
    db_session1 = get_finalyca_scoped_session(is_production_config(config))
    ...
    for df in list_grouped_holdings:
        ...
        try:
            for index, row in df_.iterrows():
                ...
                if row["InstName"] != '':
                    security_name = row["InstName"]
                elif row['Security_AssetClass'] == 'Equity' :
                    security_name = row["Company_SecurityName"]
                else:
                    # for other asset class, only issuer name is shared. So adding the Instrument Type to it
                    security_name = '{0} - {1}'.format(row["Company_SecurityName"], row["InstrumentType"])
                ...
                sql_fund = db_session.query(Fund).filter(Fund.Fund_Code == fund_code).one_or_none()
                ...
                # TODO if there is new sector which is unmapped or not found, then we will have to address the same seperately
                ...
                if isin_code != "" and isin_code != "-":
                    sql_security = db_session.query(HoldingSecurity).filter(HoldingSecurity.ISIN_Code == isin_code, HoldingSecurity.Is_Deleted != 1, HoldingSecurity.active == 1).one_or_none()

                    holding_security_type = holding_security_type_mapping[security_class.strip()].upper()

                    if not sql_security and isin_code != "":
                        ...
                        HoldingSecurity1.Updated_By = user_id
                        HoldingSecurity1.Updated_Date = TODAY
                        ...
                        db_session1.add(HoldingSecurity1)
                        db_session1.commit()

                    elif sql_security:
                        # Update the holdings security as we need the mututal fund reported information for securities,
                        # as per discussion w/ Vijay on 08/07/2023
                        sql_security.HoldingSecurity_Name = security_name
                        sql_security.ISIN_Code = isin_code
                        sql_security.Sector_Id = sql_sectorid
                        sql_security.Asset_Class = security_class
                        sql_security.Instrument_Type = instrument_type
                        sql_security.Issuer_Code = issuer_code
                        sql_security.Issuer_Name = issuer_name
                        sql_security.Is_Deleted = 0
                        sql_security.Updated_By = user_id
                        sql_security.Updated_Date = TODAY
                        sql_security.Issuer_Id = sql_issuerid
                        sql_security.MarketCap = stockrank
                        sql_security.Sub_SectorName = industry_name
                        sql_security.HoldingSecurity_Type = holding_security_type if holding_security_type else None
                        db_session.flush()
                ...
                if remark == None:
                    if isin_code != "" and isin_code != "-":
                        holding_sec_id = db_session.query(func.max(HoldingSecurity.HoldingSecurity_Id)).filter(HoldingSecurity.ISIN_Code==isin_code, HoldingSecurity.Is_Deleted != 1, HoldingSecurity.active == 1).scalar()
    ...
    if exceptions_data:
        exception_file_path = get_rel_path(output_file_path, __file__).lower().replace('holdingsupdate', 'holdingsupdate_exception')
        ...
        # TODO: Logic updated to write the csv on the given path
        ...
        #send exception file to dev team
        attachements = list()
        attachements.append(exception_file_path)
        schedule_email_activity(db_session, 'devteam@finalyca.com', '', '', F"MF - Holding - Exception file{TODAY.strftime('%Y-%b-%d')}", F"MF - Holding - Exception file{TODAY.strftime('%Y-%b-%d')}", attachements)

if __name__ == '__main__':
    USER_ID = 1
    FILE_PATH = "samples/HoldingsUpdate.csv"
    READ_PATH = "read/HoldingsUpdate.csv"
    import_mf_holding_file(FILE_PATH, READ_PATH, USER_ID)