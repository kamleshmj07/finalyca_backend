...
def import_mf_nav_file(input_file_path, output_file_path, user_id):
    ...
    header = [ "SchemeCode", "Date", "NAV", "AmfiCode", "AdjustedNAV", "Flag", "Remarks"]
    exceptions_data = []
    ...
    with open(get_rel_path(input_file_path, __file__), 'r') as f:
        ...
        for row in csvreader:
            try:
                ...
                nav = row["NAV"]
                # The net asset value is said to be “adjusted” only when there is a dividend in play. 
                # However, this does not impact the fund’s underlying performance.
                # Reference : https://mutualfunds.com/portfolio-management-channel/what-is-adjusted-nav/
                raw_nav = row["AdjustedNAV"]
                nav_date = dt.strptime(pf_date, '%d-%m-%Y')
                remark = None

                if NAV_TYPE == "P" or NAV_TYPE == "A":
                    plan_code= get_plan_codefornav(scheme_code + '_01', PRODUCT_ID)
                    ...
                    if remark == None:
                        for sql_plan1 in sql_plan:
                            ...
                            if remark == None:
                                remark = save_nav(db_session, sql_plan1.Plan_Id, NAV_TYPE, nav_date, nav, user_id, TODAY, raw_nav)
                    ...
                    if remark == None:
                        for sql_plan1 in sql_plan:
                            remark = save_nav(db_session, sql_plan1.BenchmarkIndices_Id, NAV_TYPE, nav_date, nav, user_id, TODAY, raw_nav)
        ...
        if exceptions_data:
            exception_file_path = get_rel_path(output_file_path, __file__).lower().replace('navupdate', 'navupdate_exception')
            ...
            schedule_email_activity(db_session, 'devteam@finalyca.com', '', '', F"MF - NAV - Exception file{TODAY.strftime('%Y-%b-%d')}", F"MF - NAV - Exception file{TODAY.strftime('%Y-%b-%d')}", attachements)

    write_csv(output_file_path, header, items, __file__)

if __name__ == '__main__':
    USER_ID = 1
    FILE_PATH = "samples/cmots/23012023/navupdate.csv"
    READ_PATH = "read/cmots/23012023/navupdate.csv"
    import_mf_nav_file(FILE_PATH, READ_PATH, USER_ID)