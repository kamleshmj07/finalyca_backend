...
from bizlogic.importer_helper import save_planproductmapping, get_rel_path, write_csv, get_schemecode_factsheet, get_fundid_byplanid
from bizlogic.analytics import get_risk_ratios
from bizlogic.fund_portfolio_analysis import generate_portfolio_analysis, prepare_fund_screener, update_portfolio_date

def import_pms_factsheet_file(input_file_path, output_file_path, user_id):
        ...
        with open(get_rel_path(input_file_path, __file__), 'r') as f:
            ...
            for row in csvreader:
                ...
                if Remarks == None:
                    ...
                    sql_factsheet_del = db_session.query(FactSheet)\
                                                        .filter(and_(FactSheet.Plan_Id==sql_plan_id.Plan_Id, FactSheet.TransactionDate == date, FactSheet.Is_Deleted != 1)).update(update_values)

                    db_session.commit()
                    Remarks = 'Flagged Deleted and uploaded new records Successfully.'

                    if netassetrscr == None:
                        sql_fact = db_session.query(func.max(FactSheet.TransactionDate))\
                                                    .filter(FactSheet.Plan_Id == sql_plan_id.Plan_Id, 
                                                            FactSheet.NetAssets_Rs_Cr > 0, 
                                                            FactSheet.Is_Deleted != 1, 
                                                            FactSheet.TransactionDate < date).first()

                        if sql_fact:
                            sql_facts = db_session.query(FactSheet.NetAssets_Rs_Cr)\
                                                        .filter(FactSheet.Plan_Id == sql_plan_id.Plan_Id, 
                                                                FactSheet.Is_Deleted != 1, 
                                                                FactSheet.TransactionDate == sql_fact[0]).first()

                            if sql_facts != None:
                                netassetrscr = sql_facts.NetAssets_Rs_Cr                  

                    if portfoliodividendyield == None and totalstocks == None and portfoliopbratio == None and portfolioperatio == None:
                        autocalculate = True
                    ...
                    factsheet_insert.SourceFlag = "PMS"
                    factsheet_insert.Is_Deleted = 0
                    factsheet_insert.Created_By = user_id
                    factsheet_insert.Created_Date = TODAY
                    factsheet_insert.IsRiskRatioProcessed = 1 #because below we are generating risk ratios so marking it as 1
                    
                    
                    sql_nav = db_session.query(NAV)\
                                        .filter(NAV.Is_Deleted != 1,
                                                NAV.Plan_Id == sql_plan_id.Plan_Id,
                                                NAV.NAV_Type=='P').first()

                    sql_bench_nav = db_session.query(NAV)\
                                              .filter(NAV.Is_Deleted != 1,
                                                      NAV.Plan_Id == sql_plan_id.BenchmarkIndices_Id,
                                                      NAV.NAV_Type=='I').first()

                    if sql_nav and sql_bench_nav:
                        #calculate risk analysis                    
                        sql_rate_max_date = db_session.query(func.max(RiskFreeIndexRate.Date))\
                                                            .filter(RiskFreeIndexRate.Date <= date).scalar()
                        sql_rate = db_session.query(RiskFreeIndexRate)\
                                                    .filter(RiskFreeIndexRate.Date == sql_rate_max_date).one_or_none()
                    ...
                    if sql_logics_plan.AutoPopulate:                        
                        ...
                        sql_factsheet_del = db_session.query(FactSheet)\
                                                            .filter(FactSheet.FactSheet_Id == inserted_factsheetid).update(update_values)
                        db_session.commit()

                    portfolio_date = None
                    fund_id = get_fundid_byplanid(db_session, sql_plan_id.Plan_Id)
                    ...
                    if holding_date:
                        ...
                        if portfolio_date == date:
                            update_portfolio_date(db_session, inserted_factsheetid, portfolio_date)
                        ...
                        if sql_hol_count:
                            if sql_hol_count[0][0] > 0:
                                ...
                                for T in Temp_PC_items:
                                    if T['PE'] != 0:    
                                        T["PE_Rebased"] = ((float(T['Percentage_to_AUM']) / float(sum_Percentage_to_AUM)) * 100.00) if T['Percentage_to_AUM'] else 0
                                    ...
                                    if T['PBV'] != 0:    
                                        T["PB_Rebased"] = (((float(T['Percentage_to_AUM'])) / float(sum_Percentage_to_AUM)) * 100.00) if T['Percentage_to_AUM'] else 0
                                    ...
                                    if T['mcap'] != 0:
                                        T["MCAP_Rebased"] = ((( float(T['Percentage_to_AUM'])) / float(sum_Percentage_to_AUM)) * 100.00) if T['Percentage_to_AUM'] else 0

                                        T["MCAP_Per"] = (1 /  float(T['mcap']))

                                        T["MCAP_Ratio"] = ((((( float(T['Percentage_to_AUM'])) / float(sum_Percentage_to_AUM)) * 100.00)/100) * (1 /  float(T['mcap']))) if T['Percentage_to_AUM'] else 0

                                    if T['DivYield'] != 0:
                                        T["DY_Rebased"] = ((( float(T['Percentage_to_AUM'])) / float(sum_Percentage_to_AUM)) * 100.00) if T['Percentage_to_AUM'] else 0
                                ...
                                for T in Summary_PC:#loop through each distinct plan id name date
                                    ...
                                    FactSheet.AvgMktCap_Rs_Cr : avgmktcaprscr if avgmktcaprscr else avg_mcap,
                                    FactSheet.Portfolio_Dividend_Yield : sum_DY_Ratio,
                                    FactSheet.Updated_By : user_id,
                                    FactSheet.Updated_Date : TODAY,
                                    }

                                    sql_h = db_session.query(FactSheet)\
                                                            .filter(and_(FactSheet.Plan_Id==T['Plan_Id'], FactSheet.TransactionDate == date) , FactSheet.Is_Deleted != 1)\
                                                            .update(update_values)
                                    db_session.commit()

                                    sql_factsheetdata = db_session.query(FactSheet)\
                                                                        .filter(and_(FactSheet.Plan_Id==T['Plan_Id'], FactSheet.TransactionDate == date) , FactSheet.Is_Deleted != 1).one_or_none()
...
if __name__ == '__main__':
    USER_ID = 1
    FILE_PATH = "samples/Green-Nov.csv"
    READ_PATH = "read/Green-Nov.csv"
    import_pms_factsheet_file(FILE_PATH, READ_PATH, USER_ID)
