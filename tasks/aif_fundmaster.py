...
def import_aif_fundmaster_file(input_file_path, output_file_path, user_id):
    ...
    header = [ "SCM_AMC_CD", "SCM_SCHEME_CD", "SCM_SCHEME_NAME", "Classification", "BENCHMARK NAME", "SCM_START_DATE", "SCM_MIN_PURCHASE_AMT", "SCHEME INVESTMENT STRATEGY", "Fee_Structure", "AIF_INVESTMENT_THEME", "AIF_MIN_INVESTMENT_AMOUNT", "AIF_INITIAL_DRAWDOWN", "AIF_TENURE_OF_FUND", "AIF_CURRENCY", "AIF_CATEGORY", "AIF_SUB_CATEGORY", "AIF_CLASS_OF_UNITS", "AIF_MANAGEMENT_EXPENSES", "AIF_ADMIN_EXPENSES", "AIF_SET_UP_FEE", "AIF_HURDLE_RATE", "AIF_PERFORMANCE_FEES", "Is_Active", "Remarks"]
    items = list()
    PRODUCT_ID = 5
    TODAY = dt.today()
    ...
    with open(get_rel_path(input_file_path, __file__), 'r') as f:
        ...
        for row in csvreader:
            ...
            investment_theme = row["AIF_INVESTMENT_THEME"]
            min_investment_amount = row["AIF_MIN_INVESTMENT_AMOUNT"]
            initial_drawdown = row["AIF_INITIAL_DRAWDOWN"]
            tenure_of_fund = row["AIF_TENURE_OF_FUND"]
            currency = row["AIF_CURRENCY"]
            category = row["AIF_CATEGORY"]
            sub_category = row["AIF_SUB_CATEGORY"]
            class_of_units = row["AIF_CLASS_OF_UNITS"]
            management_expenses = row["AIF_MANAGEMENT_EXPENSES"]
            admin_expenses = row["AIF_ADMIN_EXPENSES"]
            set_up_fees = row["AIF_SET_UP_FEE"]
            hurdle_rate = row["AIF_HURDLE_RATE"]
            performance_fees = row["AIF_PERFORMANCE_FEES"]
            ...
            if Remarks == None:
                ...
                if sql_fund_id:
                    ...
                    sql_fund_id.AIF_INVESTMENT_THEME = investment_theme
                    sql_fund_id.AIF_MIN_INVESTMENT_AMOUNT = min_investment_amount
                    sql_fund_id.AIF_INITIAL_DRAWDOWN = initial_drawdown
                    sql_fund_id.AIF_TENURE_OF_FUND = tenure_of_fund
                    sql_fund_id.AIF_CURRENCY = currency
                    sql_fund_id.AIF_CATEGORY = category
                    sql_fund_id.AIF_SUB_CATEGORY = sub_category
                    sql_fund_id.AIF_CLASS_OF_UNITS= class_of_units
                    sql_fund_id.AIF_MANAGEMENT_EXPENSES = management_expenses
                    sql_fund_id.AIF_ADMIN_EXPENSES = admin_expenses
                    sql_fund_id.AIF_SET_UP_FEE = set_up_fees
                    sql_fund_id.AIF_HURDLE_RATE = hurdle_rate
                    sql_fund_id.AIF_PERFORMANCE_FEES = performance_fees
                    sql_fund_id.Is_Active = is_active
                    ...
                    sql_fund_insert.AIF_INVESTMENT_THEME = investment_theme
                    sql_fund_insert.AIF_MIN_INVESTMENT_AMOUNT = min_investment_amount
                    sql_fund_insert.AIF_INITIAL_DRAWDOWN = initial_drawdown
                    sql_fund_insert.AIF_TENURE_OF_FUND = tenure_of_fund
                    sql_fund_insert.AIF_CURRENCY = currency
                    sql_fund_insert.AIF_CATEGORY = category
                    sql_fund_insert.AIF_SUB_CATEGORY = sub_category
                    sql_fund_insert.AIF_CLASS_OF_UNITS= class_of_units
                    sql_fund_insert.AIF_MANAGEMENT_EXPENSES = management_expenses
                    sql_fund_insert.AIF_ADMIN_EXPENSES = admin_expenses
                    sql_fund_insert.AIF_SET_UP_FEE = set_up_fees
                    sql_fund_insert.AIF_HURDLE_RATE = hurdle_rate
                    sql_fund_insert.AIF_PERFORMANCE_FEES = performance_fees
...
if __name__ == '__main__':
    ...
    import_aif_fundmaster_file(FILE_PATH, READ_PATH, USER_ID)